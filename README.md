...
### 📁 文件管理
- **文件上传**：支持拖拽上传、文件夹上传、批量上传
- **文件下载**：单文件下载、批量下载
- **文件删除**：单文件删除、批量删除（优化的批量 API）
- **文件重命名**：支持文件和文件夹重命名，实时名称验证
- **文件移动**：右键菜单移动到其他目录，支持批量移动
- **文件夹管理**：创建、删除、重命名文件夹
- **文件预览**：支持图片、视频、音频、PDF、文本等格式预览
- **复制链接**：一键复制文件公开访问链接

### 🎯 高级功能
- **导航历史**：前进/后退导航，类似 Finder
- **多选操作**：Cmd+Click 添加选择，Shift+Click 范围选择
- **搜索过滤**：文件名搜索、类型筛选、排序
- **队列管理**：上传/下载/移动队列，支持取消、重试
- **任务去重**：自动跳过重复任务

### 🎨 用户体验
- 原生 macOS 设计语言（SwiftUI）
- Finder 风格界面（表格视图 / 图标视图）
- 面包屑导航和路径栏
- 深色模式完美适配
- 中英文本地化支持
- 智能错误提示和操作建议
...
- **操作系统**：macOS 15.4 及以上版本
- **架构**：支持 Intel 和 Apple Silicon 处理器
...
👉 [在 App Store 下载 OwlUploader](https://apps.apple.com/cn/app/owluploader/id6746383161?l=en-GB&mt=12)
...
本项目采用 [GNU GPL v3 许可证](LICENSE) 开源。

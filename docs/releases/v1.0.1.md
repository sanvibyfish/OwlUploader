...
Fixed a critical bug where table column header sorting was broken due to circular state synchronization.

**Problem**:
- Clicking table column headers to sort files would briefly work but immediately revert to the previous sort order
- Root cause: Circular dependency between `@State sortComparators`, `@Binding sortOrder`, and `@Binding sortAscending` with three `onChange` handlers forming a feedback loop

**Solution**:
- Replaced circular sync with a computed Binding pattern that bidirectionally maps between `FileSortOrder + Bool` and `[KeyPathComparator<FileObject>]`
- Extracted mapping logic into two static methods: `comparators(from:ascending:)` and `sortOrder(from:)`
- Eliminated all `onChange` handlers, achieving single-direction data flow
- Added 18 unit tests (`FileTableSortTests.swift`) to verify mapping correctness:
  - 6 forward mapping tests (FileSortOrder → KeyPathComparator)
  - 5 reverse mapping tests (KeyPathComparator → FileSortOrder)
  - 1 round-trip consistency test
  - 6 functional sorting tests

Table sorting now works as expected: clicking a column header properly sorts files by that column, with toggle between ascending/descending on repeated clicks.
...
| `FileTableSortTests.swift` | **New** — 18 unit tests for table sorting logic |
...
| `FileTableView.swift` | **Fixed** — Column header sorting with computed Binding pattern, versioned thumbnail URL, CDN purge menu item |

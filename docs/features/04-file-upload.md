...
| 等待中 | 直接标记为取消 |
| 上传中（简单上传） | 中断当前请求 |
| 上传中（分片上传） | 中止分片上传，释放 S3 资源 |

**取消检查点**：
- 每个分片上传前检查 `Task.checkCancellation()`
- 取消后调用 `abortMultipartUpload()` 清理已上传分片

### 任务去重机制

添加文件到队列时，自动检查并跳过重复的活跃任务：

```swift
// 跳过条件：localURL 相同 且 状态为活跃（pending 或 processing）
